...
OSASCRIPT_ENTER_TEXT_SSH_CONFIG_COMMAND=$($OSASCRIPT -e "text returned of (display dialog \"Please enter an existing SSH configuration name:\" default answer \"$DEFAULT_CONFIGURATION\" with title \"Receive folder over SCP utility\" with icon note)")                                                                                                      # prompt user to enter a ssh configuration name
...
if [ -z "$OSASCRIPT_ENTER_TEXT_SSH_CONFIG_COMMAND" ]; then
  ...
  if [ -z "$OSASCRIPT_CHOOSE_LOCAL_FILEPATH_COMMAND" ]; then
    ...
    OSASCRIPT_ENTER_TEXT_REMOTE_FOLDERPATH_COMMAND=$($OSASCRIPT -e "text returned of (display dialog \"Please enter the exact path to the folder (on the remote device) that you want to transfer:\" default answer \"/home/yourUsername/AnyFolder\" with title \"Receive folder over SCP utility\" with icon note)")                                          # prompt user to enter remote folderpath

    # Continuing only if user chose to continue (OK button or enter)
    if [ -z "$OSASCRIPT_ENTER_TEXT_REMOTE_FOLDERPATH_COMMAND" ]; then
      echo "\n$($CURRENT_DATE): The script was cancelled by the user on the remote folder selection dialog.\n\n$($CURRENT_DATE): Exiting.\n"
      exit 3
    else

      # Syncronise variables
      SSH_CONFIGURATION="$OSASCRIPT_ENTER_TEXT_SSH_CONFIG_COMMAND"
      FOLDER_TO_RECEIVE="$OSASCRIPT_ENTER_TEXT_REMOTE_FOLDERPATH_COMMAND"
      LOCAL_FILEPATH="$OSASCRIPT_CHOOSE_LOCAL_FILEPATH_COMMAND"

      # Testing ...
      #echo $FOLDER_TO_RECEIVE
      #echo $SSH_CONFIGURATION
      #echo $LOCAL_FILEPATH

      echo "\n$($CURRENT_DATE): Starting folder transfer over SCP.\n"
      # Start folder transfer over $SCP to $LOCAL_HOST using $SSH_CONFIGURATION
